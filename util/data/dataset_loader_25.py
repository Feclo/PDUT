...
DATASET_PATH = {
    "ICVL512-MAT": "F:/user/wyz/dataset_4090/ICVL",
    "CAVE-16bit": "C:/checkpoint_4090/wyz_cave_dataset/CAVE",
    # "Harvard": "./datasets/HAR",
    # "NTIRE":"./datasets/NTIRE",
    # "SELF":"./datasets/SELF",
    # "KAIST":"./datasets/KAIST",
    "KAIST": "F:/user/wyz/dataset_4090/KAIST",
    "dssp": "F:/数据集/DSSP/原始数据/DSSP1"
}


# *******************************  dssp  *********************************
# ************************************************************************
def load_dssp_mat(dataset_dir, verbose=False):
    import h5py

    def icvl_mat_generator(_mat_list):
        for _mat_path in _mat_list:

            hyper = h5py.File(_mat_path)["data"]
            hyper = hyper[3:28, :, :]

            if verbose:
                tf.print("Decoding dssp MAT: <shape=", hyper.shape, ">@", _mat_path)
    ...
    dssp_dataset = tf.data.Dataset \
        .from_generator(icvl_mat_generator, args=[mat_list],
                        output_types=tf.float32, output_shapes=(25, None, None)) \
        .map(lambda _img_to_transpose: tf.transpose(_img_to_transpose, perm=[1, 2, 0])) \
        .map(lambda _img_to_norm: _img_to_norm / 1.0)
    return dssp_dataset


def load_dssp_mat_512(dataset_dir, cache_name, verbose=True):
    return load_dssp_mat(dataset_dir, verbose=verbose).cache(cache_name)
...
def load_self_datasets(dataset_dir, cache_name="cache", verbose=False):
    icvl = []
    ...
    icvl_dataset = tf.data.Dataset.from_tensor_slices(icvl) \
        .map(decode_hyper_image_from_png_list).cache(cache_name + "-self")
    return icvl_dataset


def load_self_hs_256(dataset_dir, cache_name="cache", verbose=False):
...
def load_self_hs_512(dataset_dir, cache_name="cache", verbose=False):
    icvl_dataset = load_icvl_datasets(dataset_dir, cache_name, verbose)
    icvl_dataset = icvl_dataset.flat_map(flat_map_512_patches_from_dataset)
    return icvl_dataset


def flat_map_512_overlapped_patches_from_SELF_dataset(_img):
    flat_map_operation_list = [tf.image.resize(images=_img, size=[512, 512], method=ResizeMethod.BILINEAR)]
    third_height = 464
    third_width = 434
    for i in range(0, 696, third_height):
        for j in range(0, 852, third_width):
...
def load_self_full_mat(dataset_dir, verbose=True):
    import h5py
    ...
    def icvl_mat_generator(_mat_list):
        for _mat_path in _mat_list:
            ...
            print(hyper.shape)
            if verbose:
                tf.print("Decoding SELF MAT: <shape=", hyper.shape, ">@", _mat_path)
            yield hyper
...
def load_self_full_mat_512(dataset_dir, cache_name, verbose=True):
    return load_self_full_mat(dataset_dir, verbose=verbose).flat_map(
        flat_map_512_overlapped_patches_from_SELF_dataset).cache(cache_name)


def load_self_full_mat_256(dataset_dir, cache_name, verbose=True):
    return load_self_full_mat(dataset_dir, verbose=verbose).flat_map(
        flat_map_256_overlapped_patches_from_SELF_dataset).cache(cache_name)


def load_self_full_mat_512_norm(dataset_dir, cache_name, verbose=True):
    return load_self_full_mat_512(dataset_dir, cache_name, verbose).map(lambda _x: _x / tf.reduce_max(_x))


def load_self_full_mat_512_shuffle(dataset_dir, cache_name, verbose=True):
    return load_self_full_mat_512(dataset_dir, cache_name, verbose)


def load_self_full_mat_768(dataset_dir, cache_name, verbose=True):
    return load_self_full_mat(dataset_dir, verbose=verbose).flat_map(
        flat_map_768_overlapped_patches_from_SELF_dataset).cache(cache_name)
...
def flat_map_512_overlapped_patches_from_NTIRE_dataset(_img):
    flat_map_operation_list = [tf.image.resize(images=_img, size=[512, 512], method=ResizeMethod.BILINEAR)]
    third_height = 464
    third_width = 434
    for i in range(0, 482, 81):
        for j in range(0, 512, 102):
            flat_map_operation_list.append(safe_crop_to_bounding_box(_img, i, j, 512, 512))
    return tf.data.Dataset.from_tensor_slices(flat_map_operation_list)


def flat_map_256_overlapped_patches_from_NTIRE_dataset(_img):
    flat_map_operation_list = [tf.image.resize(images=_img, size=[256, 256], method=ResizeMethod.BILINEAR)]
    for i in range(0, 482, 81):
        for j in range(0, 512, 102):
            flat_map_operation_list.append(safe_crop_to_bounding_box(_img, i, j, 256, 256))
...
def flat_map_768_overlapped_patches_from_NTIRE_dataset(_img):
    flat_map_operation_list = [tf.image.resize(images=_img, size=[768, 768], method=ResizeMethod.BILINEAR),
                               safe_crop_to_bounding_box(_img, 0, 0, 768, 768),
                               safe_crop_to_bounding_box(_img, 624, 0, 768, 768),
                               safe_crop_to_bounding_box(_img, 0, 532, 768, 768),
...
def load_NTIRE_full_mat(dataset_dir, verbose=False):
    import h5py

    def icvl_mat_generator(_mat_list):
        for _mat_path in _mat_list:
            hyper = h5py.File(_mat_path)["cube"]

            if verbose:
                tf.print("Decoding ICVL MAT: <shape=", hyper.shape, ">@", _mat_path)
            yield hyper

    def icvl_mat_generator2(_mat_list):
        for _mat_path in _mat_list:
            hyper2 = h5py.File(_mat_path)["norm_factor"]
    ...
    mat_dataset = tf.data.Dataset \
        .from_generator(icvl_mat_generator, args=[mat_list],
                        output_types=tf.float32, output_shapes=(25, None, None)) \
        .map(lambda _img_to_norm: _img_to_norm / 1) \
        .map(lambda _img_to_transpose: tf.transpose(_img_to_transpose, perm=[1, 2, 0]))
    return mat_dataset


def load_NTIRE_full_mat_512(dataset_dir, cache_name, verbose=True):
    return load_NTIRE_full_mat(dataset_dir, verbose=verbose).flat_map(
        flat_map_512_overlapped_patches_from_NTIRE_dataset).cache(cache_name)


def load_NTIRE_full_mat_256(dataset_dir, cache_name, verbose=True):
    return load_NTIRE_full_mat(dataset_dir, verbose=verbose).flat_map(
        flat_map_256_overlapped_patches_from_NTIRE_dataset).cache(cache_name)


def load_NTIRE_full_mat_512_norm(dataset_dir, cache_name, verbose=True):
    return load_icvl_full_mat_512(dataset_dir, cache_name, verbose).map(lambda _x: _x / tf.reduce_max(_x))


def load_NTIRE_full_mat_512_shuffle(dataset_dir, cache_name, verbose=True):
...
def load_cave_datasets(dataset_dir, cache_name="cache", verbose=True):
    ...
    def select_430_670nm_channels(hyper_image):
        """选择430-670nm的25个通道"""
        return hyper_image[:, :, 3:28]

    def set_patch_size(_x):
        _x.set_shape((512, 512, 25))
        return _x

    cave_dataset = tf.data.Dataset.from_tensor_slices(cave) \
        .map(decode_hyper_image_from_png_list) \
        .map(select_430_670nm_channels) \
        .map(set_patch_size).cache(cache_name + "-cave")
    return cave_dataset


def load_cave_datasets2(dataset_dir, cache_name="cache", verbose=True):
    cave = []
    for root, dirs, files in os.walk(dataset_dir):
        for _dir in dirs:
            hyper_png_path_list = tf.io.matching_files(os.path.join(root, _dir, '*.png'))
    ...
    cave_dataset = tf.data.Dataset.from_tensor_slices(cave) \
        .map(decode_hyper_image_from_png_list) \
        .map(set_patch_size).flat_map(flat_map_image_self_and_transpose).cache(cache_name + "-cave")
    return cave_dataset
...
def load_icvl_full_mat(dataset_dir, verbose=True):
    import h5py
...
def load_harvard_full_mat(dataset_dir, verbose=True):
    ...
    calibration = [
        ...
        5.8606052e-01, 6.6835907e-01, 6.7580378e-01, 6.8838983e-01, 7.0712597e-01, 7.6301691e-01,
        7.7116192e-01, 7.7402294e-01, 8.0076054e-01, 8.2445726e-01, 9.2325913e-01, 9.2700961e-01,
        9.2251713e-01, 9.1098958e-01, 9.0504569e-01, 9.0276792e-01, 1.0000000e+00, 9.5739246e-01,
        8.7174851e-01]
    calibration = tf.reshape(calibration, shape=(1, 1, 25))
    ...
    mat_dataset = tf.data.Dataset \
        .from_generator(harvard_mat_generator, args=[mat_list],
                        output_types=tf.float32, output_shapes=(1040, 1392, 25)).map(lambda _x: _x / calibration)
    return mat_dataset
...
def load_harvard_full_mat_256(dataset_dir, cache_name, verbose=True):
    return load_harvard_full_mat(dataset_dir, verbose=verbose) \
        .flat_map(flat_map_256_overlapped_patches_from_harvard_dataset) \
        .cache(cache_name)
...
def load_kaist_full_mat(dataset_dir, verbose=True):
    ...
    mat_dataset = tf.data.Dataset \
        .from_generator(kaist_mat_generator, args=[mat_list],
                        output_types=tf.float32, output_shapes=(2704, 3376, 25))
...
def load_kaist_full_mat_512(dataset_dir, cache_name, verbose=True):
    return load_kaist_full_mat(dataset_dir, verbose=verbose) \
        .flat_map(flat_map_512_patches_from_kaist_dataset).cache(cache_name)


# ************************************************************************
# *******************************  MIXED  *********************************
# ************************************************************************
def load_icvl_mat_with_kaist_512(dataset_dir, cache_name):
    cave_cache_name = "HS-KAIST-"
    ...
    cave_ds = load_kaist_full_mat_512(dataset_dir=DATASET_PATH["KAIST"], cache_name=cave_cache_name)
    icvl_ds = load_icvl_full_mat_512(dataset_dir, cache_name)
    return icvl_ds.concatenate(cave_ds)


def load_icvl_mat_with_harvard_with_kaist_512(dataset_dir, cache_name):
    cave_cache_name = "HS-KAIST-"
    cave_cache_name2 = "HS-Harvard-"
    if "train" in cache_name:
        cave_cache_name = cave_cache_name + "train"
        cave_cache_name2 = cave_cache_name2 + "train"
    elif "validation" in cache_name:
        cave_cache_name = cave_cache_name + "validation"
        cave_cache_name2 = cave_cache_name2 + "validation"
    cave_ds = load_kaist_full_mat_512(dataset_dir=DATASET_PATH["KAIST"], cache_name=cave_cache_name)
    cave_ds2 = load_harvard_full_mat_512(dataset_dir=DATASET_PATH["Harvard"], cache_name=cave_cache_name2)
    icvl_ds = load_icvl_full_mat_512(dataset_dir, cache_name)
    icvl_ds2 = icvl_ds.concatenate(cave_ds)
    icvl_ds3 = icvl_ds2.concatenate(cave_ds2)
    return icvl_ds3
...
def load_kaist_mat_with_cave_512(dataset_dir, cache_name):
    cave_cache_name = "HS-CAVE-16bit-"
    if "train" in cache_name:
        cave_cache_name = cave_cache_name + "train"
    elif "validation" in cache_name:
        cave_cache_name = cave_cache_name + "validation"
    cave_ds = load_cave_datasets2(dataset_dir=DATASET_PATH["CAVE-16bit"], cache_name=cave_cache_name)
    icvl_ds = load_kaist_full_mat_512(dataset_dir, cache_name)
    return icvl_ds.concatenate(cave_ds)


def load_NTIRE_mat_with_ICVL_256(dataset_dir, cache_name):
    cave_cache_name = "HS-icvl-256"
    if "train" in cache_name:
        cave_cache_name = cave_cache_name + "train"
    elif "validation" in cache_name:
        cave_cache_name = cave_cache_name + "validation"
    cave_ds = load_icvl_full_mat_256(dataset_dir=DATASET_PATH["ICVL512-MAT"], cache_name=cave_cache_name)
    icvl_ds = load_NTIRE_full_mat_256(dataset_dir, cache_name)
    return icvl_ds.concatenate(cave_ds)


def load_NTIRE_mat_with_cave_512(dataset_dir, cache_name):
    cave_cache_name = "HS-CAVE-16bit-"
    cave_cache_name2 = "HS-ICVL"
    if "train" in cache_name:
        cave_cache_name = cave_cache_name + "train"
    elif "validation" in cache_name:
        cave_cache_name = cave_cache_name + "validation"
    cave_ds = load_cave_hs_256(dataset_dir=DATASET_PATH["CAVE-16bit"], cache_name=cave_cache_name)
    icvl_ds = load_NTIRE_full_mat_256(dataset_dir, cache_name)
    icvll_ds = load_icvl_full_mat_256(dataset_dir=DATASET_PATH["ICVL512-MAT"], cache_name=cave_cache_name2)
    return icvl_ds.concatenate(cave_ds).concatenate(icvll_ds)
