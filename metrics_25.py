...
def ssim_metric(ground_truth, prediction):
    return tf.reduce_mean(tf.image.ssim(ground_truth, prediction[-1], max_val=1.0))
...
def psnr_hyper_metric(ground_truth, prediction):
    prediction = prediction[-1, :, :, :]
    ground_truth_channels = tf.split(ground_truth, num_or_size_splits=25, axis=-1)
    prediction_channels = tf.split(prediction, num_or_size_splits=25, axis=-1)
    psnr_sum = 0.0
    for i in range(25):
    ...
    return tf.math.divide(psnr_sum, 25.0)
