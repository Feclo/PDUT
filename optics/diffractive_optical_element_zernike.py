...
from networks.util2 import hyperboidal_phase, hyperboidal_phase_meta
...
def summary_height_map2(height_map):
    tf.summary.image(name="HeightMapNormalized", data=image_normalization(
        height_map))
...
def _phase_to_duty_func(_phase, _wavelength):
    c = [0.5477, 12.1087, 77.2867]
    duty_R = c[0] * _phase ** 2 + c[1] * _phase + c[2]
    return duty_R


def _duty_to_phase_func(resolution, duty_R, lam):
    p = [28.64, 0.3559, -0.1756, 6.735e-6, -9.069e-4, 2.968e-4, -3.999e-6, 2.173e-6, 2.74e-7, -1.541e-7]
    phi_meta_zernike = p[0] + p[1] * duty_R + p[2] * lam + p[3] * duty_R ** 2 + p[4] * duty_R * lam + p[5] * lam ** 2 + \
                       p[6] * duty_R ** 3 + p[7] * duty_R ** 2 * lam + p[8] * duty_R * lam ** 2 + p[9] * lam ** 3
    ...
    mask = tf.reshape(mask, (1, 1142, 1142, 1))
    print("mask", mask.shape)
    phi_meta_zernike = phi_meta_zernike * mask
    return phi_meta_zernike
...
class DOELayer(ABC, tf.keras.layers.Layer):
    ...
    def shift_phase_according_to_height_map(input_field, height_map, wave_lengths, wavelength_to_refractive_index_func):
        ...
        am = height_map
        phi_meta = hyperboidal_phase_meta(1142, am)
        duty_R = _phase_to_duty_func(phi_meta, 452E-9)
        duty_R = tf.reshape(duty_R, (1, 1142, 1142, 1))
        lam = wave_lengths * 1e9
        lam = lam.reshape([1, 1, 1, -1])
        lam = tf.convert_to_tensor(lam, dtype=tf.float32)
        print("duty_R", duty_R.shape)
        print("lam", lam.shape)

        phi_meta_zernike = _duty_to_phase_func(1142, duty_R, lam)
        phase_shifts = complex_exponent_tf(phi_meta_zernike)
...
def read_zernike(check_point_path):
    check_point = tf.train.latest_checkpoint(check_point_path)
    zernike_a = tf.train.load_variable(check_point,
                                       "doe_layer/weight_height_map_sqrt/.ATTRIBUTES/VARIABLE_VALUE")
    return zernike_a
...
class HeightMapDOELayer(DOELayer):
    def __init__(self, wave_length_list, wavelength_to_refractive_index_func, block_size=1, height_map_initializer=None,
        ...
        self.height_constraint_max = _phase_to_height_with_material_refractive_idx_func(
            math.pi, 700 * 1e-9, wavelength_to_refractive_index_func)
...
class PhaseDOELayer(DOELayer):
    def __init__(self, wave_length_list, wavelength_to_refractive_index_func, block_size=1, height_map_initializer=None,
                 ...
                 alpha_blending_start_epoch=5, alpha_blending_end_epoch=25, name="HeightMapDOELayer"):
        super(PhaseDOELayer, self).__init__(name=name)
        self.wave_length_list = wave_length_list
        self.wavelength_to_refractive_index_func = wavelength_to_refractive_index_func
        self.block_size = block_size
        self.height_map_initializer = height_map_initializer
    ...
    def build(self, input_shape):
        _, height, width, _ = input_shape
        self.height_map_shape = [8]
        if self.height_map_initializer is None:
            init_height_map_value = np.ones(shape=self.height_map_shape, dtype=np.float32) * 0e-4
    ...
    def preprocess_height_map(self, training=None, testing=None):
        height_map = self.weight_height_map
        return height_map
    ...
    def preprocess_height_map(self, training=None, testing=None):
        ...
        def _quantized_path(_weight, _round_op, _quantization_level_cnt=4, _adaptive=False):
            ...
            if _adaptive and (testing is not True):
                ...
                self.add_loss(0.01 * adaptive_quantization_loss)
...
class QuadSymmetricQuantizedHeightMapDoeLayer(QuantizedHeightMapDOELayer):
    ...
    def preprocess_height_map(self, training=None, testing=None):
        ...
        height_map_full = tf.expand_dims(height_map_full, axis=-1)
        return height_map_full
...
class Rank1HeightMapDOELayer(DOELayer):
    ...
    def preprocess_height_map(self, training=None, testing=None):
        height_map_mul = tf.matmul(self.weight_map_row, self.weight_map_column)
        height_map = 1.125 * 1e-6 * tf.sigmoid(height_map_mul)
        height_map = tf.expand_dims(height_map, 0)
        height_map = tf.expand_dims(height_map, -1)
        return height_map
