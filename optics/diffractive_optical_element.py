...
class DOELayer(ABC, tf.keras.layers.Layer):
    ...
    def shift_phase_according_to_height_map(input_field, height_map, wave_lengths, wavelength_to_refractive_index_func):
        ...
        phi3 = wave_numbers * delta_n * height_map ** 2
        sio.savemat(
            'heightmap.mat',
            {"heightmap": height_map},
            do_compression=False)

        p = [6.051, -0.02033, 2.26, 1.371E-5, -0.002947, 0.797]

        duty = height_map * 0.8 * 1e6 * 0.3 + 0.5
        duty = height_map * 0.3 + 0.5
        duty = height_map * 0.62 + 0.2
        wave_lengthsm = wave_lengths * 1e9
        wave_lengthsm = wave_lengthsm.reshape([1, 1, 1, -1])
        phase = p[0] + p[1] * wave_lengthsm + p[2] * duty + p[3] * wave_lengthsm ** 2 + p[4] * wave_lengthsm * duty + p[
            5] * duty ** 2
