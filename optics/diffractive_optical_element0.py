...
from networks.util2 import hyperboidal_phase
...
class DOELayer(ABC, tf.keras.layers.Layer):
    ...
    def shift_phase_according_to_height_map(input_field, height_map, wave_lengths, wavelength_to_refractive_index_func):
        ...
        """
        am = height_map
        phi2 = hyperboidal_phase(1024, am)
        phi2 = phi2 % (2 * math.pi)
        height_shuangqu = _phase_to_height_with_material_refractive_idx_func(phi2, 633E-9,
                                                                             wavelength_to_refractive_index_func) + 1e-3
        height_shuangqu = tf.expand_dims(height_shuangqu, -1)
        summary_height_map2(height_shuangqu)
        ...
        phi22 = wave_numbers * delta_n * height_shuangqu
        phase_shifts = complex_exponent_tf(phi22)
