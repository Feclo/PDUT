...
from summary_25 import summary_hyper_spec_image, image_normalization
...
from .sensor_25 import Sensor
from .util_25 import depth_dep_convolution, image_convolve_with_psf, complex_exponent_tf
...
class Camera(tf.keras.layers.Layer):
    ...
    def __init__(self, wave_resolution, wave_length_list, sensor_distance, sensor_resolution, input_sample_interval,
                 sensor: Sensor = None, doe_layer: DOELayer = None, input_channel_num=25,
                 noise_model=gaussian_noise, depth_map=None, depth_list=None, otfs=None,
                 should_use_planar_incidence=True, should_depth_dependent=False,
        ...
        Args:
            ...
            input_channel_num: The number of input channel. Default is 25.
    ...
    def generate_psf_from(self, input_fields, training=True, testing=None):
        ...
        for depth_idx, input_field in enumerate(input_fields):
            ...
            aperture = 1142
            period = 350e-9
            sensor_pixel = 3.45e-6
            magnification = 10
            mag_width = int(np.round(aperture * period * magnification / sensor_pixel))

            mag_intensity = tf.image.resize(psf, [mag_width, mag_width], method='bilinear')
            denom = tf.math.reduce_sum(mag_intensity, axis=[1, 2], keepdims=False)
            denom = denom[tf.newaxis, tf.newaxis, :]
            mag_intensity = mag_intensity * tf.math.reduce_sum(psf, axis=[1, 2], keepdims=True) / denom
            psf = mag_intensity
            print("PSFshapewwwwwwwwww：", psf.shape)
    ...
    def get_pre_sensor_image(self, input_img, psfs):
        ...
        if self.flag_depth_dependent:
            ...
            print("sensor_img22222222222222:", sensor_img)
    ...
    def call(self, inputs, training=None, testing=None, **kwargs):
        ...
        print("inputsABABABABAB:", inputs.shape)

        print("psfs:", psfs)
        print("ababababababababababababab")
        pre_sensor_image = self.get_pre_sensor_image(input_img=inputs, psfs=psfs)
        print("pre_sensor_image:", pre_sensor_image.shape)
        ...
        if self.flag_resize_image_to_psf_size:
            ...
            print("sensor_image:", sensor_image)

        if self.sensor is not None:
            tf.summary.image(name="SensorImage", data=sensor_image, max_outputs=1)
        psfs = tf.transpose(tf.squeeze(psfs, 0), perm=[2, 0, 1, 3])
        psfs = tf.image.resize(psfs, self.sensor_resolution,
                               method=tf.image.ResizeMethod.BILINEAR)

        return sensor_image, psfs
...
class PSFFixedCamera(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self._input_shape = input_shape
        self.psf = self.add_weight(name="PSF", shape=(1, 512, 512, 25),
                                   dtype=tf.float32,
                                   initializer=tf.constant_initializer(
                                       value=sio.loadmat(
                                           "psf_crop_all_512_25_cap_plan_zhunzhitoujing.mat")["psf_crop_all_512_25"]),
                                   trainable=False)
        self.psf = tf.transpose(self.psf, perm=[1, 2, 0, 3])
