...
class DOELayer(ABC, tf.keras.layers.Layer):
    ...
    def shift_phase_according_to_height_map(input_field, height_map, wave_lengths, wavelength_to_refractive_index_func):
        ...
        wave_lengthsm = wave_lengths * 1e9
        x = wave_lengthsm.reshape([1, 1, 1, -1])
        y = height_map * 100 + 50

        p = [63.72, -0.4182, 0.5153, 0.001088, -0.002326, 0.001555, -1.173e-06, 1.327e-06, 1.045e-05, -2.888e-05,
             4.232e-10, 8.836e-10, -1.277e-08, 1.898e-08, 3.534e-08]
        phase_f = p[0] + p[1] * x + p[2] * y + p[3] * x ** 2 + p[4] * x * y + p[5] * y ** 2 + p[6] * x ** 3 + p[
            7] * x ** 2 * y + p[8] * x * y ** 2 + p[9] * y ** 3 + p[10] * x ** 4 + p[11] * x ** 3 * y + p[
                      12] * x ** 2 * y ** 2 + p[13] * x * y ** 3 + p[14] * y ** 4
        ...
        phase_shifts = complex_exponent_tf(phase_f)
...
class QuantizedHeightMapDOELayer(DOELayer):
    def __init__(self, wave_length_list, wavelength_to_refractive_index_func, height_map_initializer=None,
        ...
        self.quantization_height_base_wavelength = 633e-9
    ...
    def preprocess_height_map(self, training=None, testing=None):
        ...
        final_processed_height_map = _full_precise_path(self.weight_height_map)
