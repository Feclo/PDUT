...
from constants import SRF_GS3_U3_41S4C_BGR_31_CHANNEL_400_700NM, \
    ...
    SRF_BFS_U3_88S6C_BGR_31_CHANNEL_460_700NM, SRF_BFS_U3_88S6C_BGR_31_CHANNEL_430_670NM, \
    ...
    SRF_BFS_U3_51S5PC_BGR_31_CHANNEL_400_700NM, SRF_BFS_U3_51S5PC_BGR_31_CHANNEL_460_700NM

SRF_BGR_31_CHANNEL_400_700_NM = SRF_BFS_U3_88S6C_BGR_31_CHANNEL_430_670NM
...
def mask_and_reshape(tensor, mask, half_height, half_width):
    ...
    tensor = tf.reshape(tensor, [half_height, half_width, 1, -1])
    return tensor
...
def inverse_simulated_gray_channel_camera_spectral_response_function(input_gray_image):
    ...
    gray_channel_expanded = gray_channel_expanded * tf.reduce_sum(
        SRF_GRAY_31_CHANNEL_400_700_NM) / SRF_GRAY_31_CHANNEL_400_700_NM
...
def simulated_rgb_camera_spectral_response_function_lost(hyper_spectral_image):
    channel_num = 25
    masked_response_function = SRF_BGR_31_CHANNEL_400_700_NM
...
def simulated_rgb_camera_spectral_response_function(hyper_spectral_image):
    channel_num = 25
    masked_response_function = SRF_BGR_31_CHANNEL_400_700_NM
    SRF_array = np.array(masked_response_function)
    result_array = np.flip(SRF_array, axis=0)
    result_list = result_array.tolist()
    masked_response_function = result_list
...
def simulated_rggb_bayer_raw_1_channel_response_function(hyper_spectral_image):
    ...
    red, green, blue = tf.split(rgb_image, num_or_size_splits=3, axis=-1)
...
def simulated_rggb_bayer_separated_4_channel_response_function(hyper_spectral_image):
    rgb_image = simulated_rgb_camera_spectral_response_function(hyper_spectral_image)
    _, height, width, _ = hyper_spectral_image.shape
    rgb_image = tf.transpose(rgb_image, perm=[1, 2, 3, 0])
    ...
    r = mask_and_reshape(red, RGGB_BAYER_PATTERN_BOOLEAN_MASK[height].r, half_height, half_width)
    ...
    combined_4_channel_image = tf.concat([r, g1, g2, b], axis=2)
